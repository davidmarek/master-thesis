...
Ovšem při hovoru s~uživatelem není možné si dovolit dlouhé pauzy, protože působí rušivě.
...
Další knihovny jsou například Bayes Net Toolbox pro Matlab (BNL)~\cite{bnl} nebo Probabilistic Network Library (PNL, \url{http://sourceforge.net/projects/openpnl/}).
...
Dále existují knihovny používající místo variačních metod vzorkování.
Příkladem je knihovna PyMC~\cite{patil2010pymc}. 
Díky využití metody Monte Carlo je jednodušší počítání se složitějšími distribucemi, ovšem na druhou stranu platíme mnohdy pomalou konvergencí.
...
Faktory je třeba úsporně reprezentovat.
Každá zpráva, každé pravděpodobnostní rozdělení je implementováno pomocí faktoru.
...
Pole využívají implementaci z~knihovny Numpy, díky ní jsou matematické operace napsané v~rychlejším jazyku než je Python (C, Fortran) a jsou navíc vektorizované.
...
Krok určuje o~kolik hodnot v~tabulce se musíme posunout, abychom se dostali na další hodnotu této proměnné se zachováním hodnot všech následujících proměnných.
...
Jednotlivé prvky nového faktoru jsou pak výsledkem aplikace operace na prvky ze vstupních faktorů sdílejících přiřazení společných proměnných.
Příklad násobení takových dvou faktorů je v~tabulce~\ref{tab:facmul}.
...
Faktory sdílí pouze proměnnou $Y$, takže je potřeba pronásobit všechna přiřazení z~$f_1$ se všemi přiřazeními z~$f_2$, která mají stejnou hodnotu $Y$.
...
Nyní představíme efektivní implementaci třetí možnosti, tedy aplikaci operace na dva faktory s~různými doménami (algoritmus \ref{alg:apop}).
...
Tento algoritmus pracuje přímo nad polem a jeho výhodou je, že nepotřebuje ke svému výpočtu znát jednotlivá přiřazení proměnných.
...
\subsection{Marginalizace proměnných ve faktoru}

Další důležitou a často používanou operací při počítání s~faktory je marginalizace.
Na vstupu je faktor a podmnožina proměnných z~domény faktoru, která má zůstat po vysčítání zbytku.
...
Postup marginalizace je zapsán v~algoritmu~\ref{alg:marg}.
...
Vrcholy pro faktory reprezentují pouze samotné faktory, ze~kterých se skládá sdružená distribuce.
...
Díky tomu, že na posílání zpráv se podílí odesílatel i~příjemce, můžeme kombinovat různé vrcholy. 
Stačí, pokud se dohodnou na stejném formátu zprávy.
...
Pro odeslání zpráv do všech vrcholů slouží metoda \texttt{send\_messages}.
...
Nejprve je třeba mít možnost nastavit pozorované proměnné.
K~tomu slouží metoda \texttt{observed}.
...
Některé dialogové strategie potřebují jen jednu nejpravděpodobnější hodnotu, ale jiné mohou počítat se seznamem nejpravděpodobnějších možností. 
...
V~modulu \texttt{alex.infer.node} jsou implementovány vrcholy pro práci s~Dirichletovskými parametry.
...
Pro zprávu z~faktoru do parametru je třeba provést aproximaci sdružené pravděpodobnostní distribuce a z~ní odvodit nové parametry (popsáno v~algoritmu~\ref{alg:ep} na straně~\pageref{alg:ep}).
...
Předpokládáme, že již máme vytvořený graf z~vrcholů představených v~předchozí sekci.
...
Pro to existuje několik metod lišících se podle typu grafu.
...
Tato metoda pak provádí inferenci, je možné zadat počet iterací, v~případě inference v~dynamické síti i~zadat, od které vrstvy se má inference provádět.
...
Rozešleme z~nich vždy zprávu do souseda, ze~kterého ještě zpráva nepřišla a snížíme jeho počet chybějících zpráv.
...
Po vypočtení marginální pravděpodobnosti ve stromě o~jednom vrcholu můžeme zase přidávat vrcholy v~obráceném pořadí, než v~jakém jsme je odebírali.
...
Čím dál do historie jdeme, tím menší vliv naše nová pozorování budou mít na vrcholy v~dané vrstvě.
...
V~příkladu~\ref{lst:facmul} je ukázáno násobení dvou faktorů.
...
\PY{g+go}{\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}\PYZhy{}}
\end{Verbatim}
\caption{Násobení faktoru konstantou}
\label{lst:facmulk}
\end{example}
...
Předvedeme si ji na faktoru pro pozorování (příklad~\ref{lst:facmar}), ze~kterého chceme získat pouze marginální pravděpodobnosti pozorování.
...
\end{Verbatim}
\caption{Nejpravděpodobnější přiřazení}
\label{lst:facmop}
\end{example}
...
Model bude simulovat dvě obrátky, v~každé bude jedna skrytá a jedna pozorovaná proměnná.
...
Po vytvoření grafického modelu je dalším krokem nastavení hodnot pozorovaných přiřazení (příklad~\ref{lst:facobs}).
...
\PY{p}{\PYZcb{}}\PY{p}{)}
\end{Verbatim}
\caption{Nastavení hodnot pozorovaných přiřazení}
\label{lst:facobs}
\end{example}
...
Ve výsledku (příklad ~\ref{lst:facres}) vidíme, že ve~druhé obrátce z~pozorování sice nezískáme žádnou informaci, ale díky vysoké pravděpodobnosti přechodu zůstala pravděpodobnost čínského jídla stále vysoká.
...
Pro každou kombinaci hodnot rodičů existuje jedno Dirichletovské rozdělení popisující apriorní rozdělení nad parametry faktoru.
V~tomto případě je to skrytá proměnná v~druhé obrátce.
Parametry mohou být buď nastaveny bez apriorní znalosti, pak jsou všechny rovny jedné, anebo můžeme nějakou naši apriorní znalost přidat.
V~tomto případě budeme předpokládat, že se cíl spíše nemění.

Pozorování budou nastavena tak, že je zřejmé, který typ jídla uživatel požaduje.
...
Představme si, že máme další hovor o~dvou obrátkách, avšak s~jinými pozorovanými hodnotami.
Stačí nastavit nová pozorování, znovu inicializovat zprávy a spustit inferenci.
Výhodou je, že není třeba znovu vytvářet síť.
Navíc počítáme už s~upraveným Dirichletovským parametrem pro přechodovou pravděpodobnost.
...
Systém rozpoznává devět různých slotů, některé z~nich se sestávají z~podslotů.
Tyto sloty jsou
\begin{itemize}
...
\end{itemize}
Navíc se sloty \emph{time} a \emph{date} skládají z~dalších pěti, respektive čtyř slotů (např. \emph{time.hour}, \emph{time.minute}, \emph{date.day}).
...
Organizátoři poskytli čtyři datové sady pocházející ze tří různých dialogových systémů.
...
Parametr $\theta_t$ určuje, jak moc bude systém zapomínat, v~případě hodnoty blízké~1 systém téměř nezapomíná a jakmile nějakou hodnotu pozoruje, bude jej těžké přesvědčit o~čemkoliv jiném.
V~opačném případě systém zapomíná a~i~z~téměř jisté důvěry v~přiřazení stavu může během pár obrátek dojít do stavu, kdy mají všechny možné přiřazení stejnou pravděpodobnost.
...
Pro inferenci bylo použito LBP, a protože možných přiřazení slotů bylo v~řádu stovek, všechny nepozorované hodnoty byly staženy do jedné speciální hodnoty a~jejich pravděpodobnost byla počítána pouze dohromady.

Popis použitého systému a jeho srovnání s diskriminativním systémem bylo publikováno v samostatném článku~\cite{zilka2013}.

\subsection{Evaluace}

Použitý model byl testován na čtyřech testovacích datových sadách s~live daty a na dvou testovacích datových sadách s~batch daty.
...
Zde byl rovnocenným soupeřem většině ostatních systémů (viz tabulka~\ref{t:DSTC:ranking}).
V oficiálních výsledcích bude generativní systém veden jako druhý systém týmu číslo 3.
