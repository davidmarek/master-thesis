...
Každý dialogový systém obsahuje dialogový manager, který vykonává akce dialogového systému na základě dialogového stavu.
Dialogový stav reprezentuje všechny informace poskytnuté jak uživatelem, tak systémem.
Systém pro odhad stavu slouží k udržování stavu dialogu a integraci nových informací ze systému porozumění.
Pro efektivní práci dialogového manageru je důležité co nejpřesněji modelovat pravděpodobnostní rozdělení definované nad všemi stavy dialogu. 
Techto stavů může být velké množství, proto se většinou používají aproximativní techniky.
...
Výsledkem bude knihovna použitelná v systému pro odhad stavu reálného dialogového systému.


Klíčová slova: dialogový systém, odhad dialogového stavu, aproximativní bayesovské metody
...
Title: Approximative Bayes methods for belief monitoring in spoken dialogue systems
...
Every dialog system contains a dialog manager, which given a dialog state issues system actions.
Dialog state represents all of the information received from user and system.
Dialog state tracker's purpose is to maintain a dialog state and integrate new information from spoken language understanding module.
...
In this thesis we will discuss an implementation of approximative Bayes methods for belief state monitoring.
The result will be a library for dialog state monitoring in real dialog systems.

Keywords: dialog system, belief state monitoring, approximative Bayes methods
