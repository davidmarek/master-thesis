...
Podmíněná pravděpodobnostní distribuce (CPD) proměnné $X$ popisuje pravděpodobnost proměnné $X$ dáno jejími rodiče, $P(X \mid parents(X))$.
...
Konečně doporučení, které student obdrží, je podmíněně nezávislé na všech ostatních proměnných, dáno studentovou známka.
...
Posílání zpráv budeme iterovat dokud zprávy nekonvergují, anebo můžeme nastavit pevný počet iterací.
...
Při posílání zprávy z faktoru $f$ do proměnné musíme navíc vynásobit příchozí zprávy hodnotou $f$, následně vysčítat všechny proměnné, kromě té, které zprávu posíláme.
...
Pro nalezení hodnoty $K$ opět použijeme srovnání momentů, v tomto případě nultých momentů:
\begin{equation}
    \int \tilde{f}_j (\vec{X}) q^{\backslash j}(\vec{X}) \mathrm{d}\vec{X} = \int f_j(\vec{X}) q^{\backslash j}(\vec{X}) \mathrm{d}\vec{X}
\end{equation}
Vidíme, že pravá strana je rovna $Z_j$ a tedy platí rovnost $K = Z_j$.
Díky tomu můžeme $K$ nalézt přímo výpočtem integrálu~(\ref{eq:zj}).
Bez tohoto poznatku bychom se dostali do problému, protože pro výpočet $K$ by bylo potřeba $\tilde{f}_j$, které ale bez $K$ neznáme.
