...
Faktory jsou reprezentovány třídou \texttt{Factor} v modulu \texttt{bn/factor.py}, která podporuje všechny základní matematické operace a také řadu specifických operací jako je marginalizace, normalizace, nastavení faktoru na pozorovanou hodnotu, vybrání nejpravděpodobnějšího přiřazení, atd.
...
Implementace vrcholů je v modulu \texttt{bn/node.py}.
...
V modulu \texttt{bn/node.py} jsou implementované vrcholy pro práci s dirichletovskými parametry.
...
V modulu \texttt{bn/lbp.py} je implementována třída \texttt{LBP}, která implementuje iterativní algoritmus pro aktualizaci grafického modelu a také obsahuje několik implementací strategií pro různé typy grafů.
...
V příloze v adresáři \texttt{dstc/} jsou všechny potřebné soubory pro spuštění inference na datové sady z DSTC.
V adresáři \texttt{dstc/data} jsou všechny datové sady.
Zdrojové kódy jsou v \texttt{dstc/src}.

Kód pro načtení dat je v modulu \texttt{loader.py}, data byla poskytnuta ve formátu JSON~\cite{crockford2006application}.
Skript \texttt{system\_dm.py} obsahuje kód pro vytvoření grafického modelu z dat a inferenci.
...
Pro všechny testovací datové sady jsou poskytnuty konfigurační soubory (např. \texttt{test1.conf}).

Pro získání metrik z vypočtených dat je třeba spustit skripty poskytnuté organizátory.
Celý proces byl zjednodušen přidáním skriptu \texttt{test\_run.sh}, který jako jediný parametr očekává konfigurační soubor a sám spustí inferenci a nakonec i vypíše hodnotu metrik.

Bližší dokumentace používání, i s příklady je obsažena v příloze.
